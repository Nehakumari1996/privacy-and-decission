 ...
 */
public class SimpleRegionGrowing 
  {
 ...
 */
  private PlanarImage preprocess(PlanarImage input)
    ...
    if (input.getNumBands() > 1)
      ...
      double[][] matrix = {{ 0.01, 0.01, 0.01, 0.01 }};
 ...
 * @throws IOException 
  */
  public PlanarImage getOutput() throws IOException
    {
    ...
    System.out.println("the out put image is "+tiledImage);
...
public static void do_RegionGrowing(String imgname,String oppath) throws IOException, InterruptedException {
	
	System.setProperty("com.sun.media.jai.disableMediaLib" ,"true");
	
	BufferedImage renderedImage = ImageIO.read(new File(imgname));
	PlanarImage planarImage = PlanarImage.wrapRenderedImage(renderedImage);
	
	
	SimpleRegionGrowing s=new SimpleRegionGrowing(planarImage, true);
	s.run();
	
	
	new DisplayDEM(s.getOutput(),oppath);
  ...
  {
	do_RegionGrowing("op.png","reg.png");
}
